            ...
            for node in nodes:
                ...
                if url and not url.startswith("centric://"):
                    price_map[url] = {
                        ...
                        "supplier":   node.get("p_purchaseorder_supplier_username", ""),
                        "supplier_email": node.get("p_purchaseorder_supplier_user_email", ""),
                        "factory":    node.get("po_factory_lookup", ""),
                        "department": node.get("P_PO_CC_DeptNumber", ""),
                        "country":    node.get("p_purchasedorder_country_lookup", ""),
                        "incoterm":   node.get("p_purchasedorder_incoterm_code_lookup", ""),
...
def main():
    ...
    for po_num, url in sorted(po_to_url.items(), key=lambda x: x[0]):
        ...
        rows.append({
            "PO Number":        po_num,
            "Supplier Contact": detail.get("supplier", ""),
            "Supplier Email":   detail.get("supplier_email", ""),
    ...
    df = pd.DataFrame(rows, columns=[
        "PO Number", "Supplier Contact", "Supplier Email",
        "Style Code", "Season", "Department", "Factory", "Country", "Incoterm", "Qty",
